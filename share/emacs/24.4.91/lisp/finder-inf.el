...
(setq finder-keywords-hash
      #s(hash-table size 487 test eq rehash-size 1.5 rehash-threshold 0.8 data (abbrev (skeleton pcomplete emacs icomplete hippie-exp expand edmacro dabbrev completion autoarg) convenience (table picture emacs makeinfo flyspell keypad cua-base calc diff-mode compare-w erc winner windmove vcursor tmm ruler-mode repeat pcomplete minibuf-eldef mb-depth linum kmacro json imenu ido ibuffer hippie-exp frameset follow filesets filecache ffap ebuff-menu desktop delsel dabbrev completion chistory calculator bs autorevert autoinsert align) languages (sgml-mode xscheme vhdl-mode verilog-mode vera-mode tcl sql simula sh-script scheme ruby-mode python ps-mode perl-mode pascal opascal octave modula2 mixal-mode meta-mode m4-mode ld-script js idlwave icon fortran flymake f90 executable dcl-mode cwarn cperl-mode cfengine cc-mode cc-guess cap-words bat-mode autoconf asm-mode antlr-mode ada-mode nxml smie emacs erc tempo skeleton paren info-look allout align) lisp (inversion cedet xscheme scheme inf-lisp hideshow unsafep trace testcover tabulated-list smie shadow re-builder pp emacs macroexp ewoc ert elp elint eieio eieio-core edebug debug checkdoc check-declare benchmark strokes profiler isearchb color cmuscheme align) outlines (org hideshow hideif reveal outline foldout elide-head allout allout-widgets) wp (underline two-column tildify emacs table sgml-mode refill picture page-ext nroff-mode ispell enriched org ebnf2ps nxml handwrite whitespace ps-print printing allout) pgp (gnus epg epa emacs allout) gnupg (gnus epg epa emacs allout) comm (url sgml-mode dns-mode zeroconf webjump tramp tls telnet socks soap-client secrets rlogin rcirc net-utils ldap gnutls eudc dns dig dbus ange-ftp sql bindat erc talk shadowfile notifications master kermit ansi-color) processes (url tramp sql inf-lisp idlwave grep compile bindat eshell erc term shell server proced pcomplete emacs emacs-lock dirtrack cmuscheme ansi-color) terminals (tramp xt-mouse vt-control ansi-color) services (ansi-color) help (emacs woman novice man makesum informat info info-look icomplete help-at-pt finder ehelp) files (url po compare-w wdired view emacs shadowfile recentf locate ffap doc-view arc-mode) archives (arc-mode) msdog (arc-mode) editing (hideshow dcl-mode arc-mode) major-mode (dcl-mode arc-mode) extensions (gnus tq tabulated-list rx ring regi regexp-opt emacs ewoc eldoc cl cl-lib benchmark avl-tree calc cookie1 type-break tree-widget thingatpt tempo strokes skeleton ido emacs-lock electric ehelp array) emulations (emacs viper edt cua-base hl-line delsel autoarg) mouse (artist goto-addr browse-url xt-mouse emacs thingatpt t-mouse strokes msb mouse-drag mouse-copy ffap avoid) hardware (zeroconf dbus vt100-led emacs flow-ctrl battery) internal (emacs warnings syntax smie disass select novice loadhist gs delim-col) bookmarks (saveplace bookmark) placeholders (saveplace bookmark) annotations (bookmark) tools (semantic which-func make-mode hideshow gud grep glasses gdb-mi flymake etags ebrowse cpp compile bug-reference asm-mode trace re-builder package emacs ert elp eieio edebug debug copyright check-declare reporter mail-utils vc smerge-mode log-view emerge ediff diff cvs-status add-log time-stamp tempo speedbar skeleton sb-image pcomplete imenu format-spec find-file ezimage elide-head dframe calculator) i18n (po gnus emacs double descr-text) data (url remember page-ext snmp-mode org gnus bindat timeclock xml whitespace jka-compr emacs hexl hex-util cdl) faces (enriched gnus m4-mode ld-script cwarn cpp erc emacs paren hl-line hilit-chg hi-lock face-remap descr-text color) color (color) hex (color) rgb (htmlfontify color) hsv (color) hsl (color) cie-lab (color) background (color) mule (emacs ps-print) multilingual (emacs descr-text) character\ composition (emacs) expand (dabbrev) completion (crm dabbrev) unicode (emacs descr-text) file (dns-mode speedbar sb-image ezimage dframe) tags (ede semantic ebrowse speedbar sb-image ezimage dframe) dired (wdired emacs) window (emacs windmove follow) drag (emacs) drop (emacs) pdf (doc-view) ps (doc-view) dvi (doc-view) font (ps-print emacs) system-font (emacs) tool-bar-style (emacs) unix (ispell telnet rlogin sh-script make-mode gud gdb-mi executable cc-mode emerge ediff diff woman tar-mode sort proced emacs lpr locate find-lisp find-dired) mail (mairix imap goto-addr mh-e gnus unrmail undigest uce supercite smtpmail sendmail rmail rfc822 rfc2368 reporter mspools metamail mail-utils mailclient mailalias mailabbrev hashcash footnote feedmail emacs fortune time-date epa) message (gnus feedmail epa) face\ remapping (face-remap) display (hi-lock follow face-remap) user\ commands (face-remap) hypermedia (url sgml-mode css-mode soap-client quickurl browse-url org nxml emacs htmlfontify ffap) matching (regi re-builder thingatpt soundex misearch emacs hi-lock find-file ffap) filesets (filesets) c (hideshow hideif flymake cwarn cpp cmacexp cc-mode cc-guess emacs find-file) folding (foldout) minor-mode (hi-lock follow) frames (winner emacs talk hl-line) generic (emacs) comment (emacs) font-lock (emacs) colour (htmlfontify) html (shr shr-color eww gnus htmlfontify) markup (htmlfontify) etags (htmlfontify) buffer (msb ibuffer) multimedia (gnus metamail thumbs mpc emacs image-file image-dired iimage) docs (texnfo-upd texinfo texinfmt makeinfo lisp-mnt checkdoc info-xref) keyboard (keypad cua-base kmacro) md4 (md4) utilities (midnight) menu (erc msb) uncomment (emacs) desktop (notifications) notifications (notifications) password (secrets password-cache) cache (password-cache) passphrase (secrets password-cache) key (password-cache) print (handwrite ps-print printing) postscript (gnus ps-mode ebnf2ps handwrite ps-print printing) bdf (ps-print) multibyte (ps-print) vi (repeat) repeat (repeat) minibuffer (crm savehist emacs) scroll (scroll-all) crisp (scroll-all) brief (scroll-all) lock (scroll-all) spreadsheet (testcover ses) dijkstra (ses) gpm (t-mouse) linux (t-mouse) news (newsticker netrc gnus uudecode supercite metamail mail-utils footnote binhex time-date timezone) timers (type-break) virtual\ cursor (vcursor) environment (wdired) renaming (wdired) movement (windmove) xml (nxml xml) irc (org erc) chat (erc) client (erc) internet (erc) button (erc) url (url vc erc) regexp (nxml erc) logging (erc) timestamp (erc) calendar (org gnus todo-mode timeclock calendar icalendar diary-lib) holidays (calendar) util (time-date parse-time) todo (remember todo-mode) vc (ede url vc smerge-mode pcvs log-view log-edit emerge ediff diff diff-mode cvs-status compare-w add-log) pcl-cvs (pcvs log-edit cvs-status) cvs (smerge-mode pcvs log-edit cvs-status) status (cvs-status) tree (avl-tree cvs-status) patch (diff-mode) diff (diff-mode) comparing (ediff) merging (ediff) patching (ediff) commit (log-edit) log (log-edit) release\ management (pcvs) revision\ control (smerge-mode) merge (smerge-mode) diff3 (smerge-mode) conflict (smerge-mode) games (zone tetris studly spook solitaire snake pong mpuz morse life landmark hanoi gomoku gametree gamegrid fortune dunnet doctor dissociate decipher cookie1 bubbles blackbox animate 5x5) puzzles (5x5) utils (fortune) cursive\ writing (handwrite) neural\ network (landmark) adaptive\ search (landmark) chemotaxis (landmark) ccl (emacs) character\ set (emacs) coding-system (emacs) multibyte\ character (emacs) syntax (semantic mode-local emacs) category (emacs) fontset (emacs) isearch (emacs) tex (tildify texnfo-upd texinfo texinfmt tex-mode reftex refbib bibtex bibtex-style org meta-mode emacs) iso (emacs) latin (emacs) japanese (emacs) kinsoku (emacs) coding\ system (emacs) latex (bibtex emacs) charset (emacs) diagnosis (emacs) input\ method (emacs) quail (emacs) tit (emacs) cxterm (emacs) normalization (emacs) cua (cua-base) mark (cua-base) binhex (gnus binhex) maint (texnfo-upd texinfo texinfmt gulp edebug debug copyright checkdoc check-declare emacs reporter) email (rmail feedmail) queue (feedmail) sendmail (feedmail) spray (feedmail) smtp (feedmail) draft (feedmail) hashcash (gnus hashcash) history (mail-utils) mime (gnus metamail) spam (mh-e gnus rmail) filter (rmail) rmail (rmail) uce (uce) unsolicited\ commercial\ email (uce) uudecode (gnus uudecode) data\ structures (avl-tree) avl (avl-tree) oo (inversion cedet eieio eieio-core chart) chart (chart) graph (chart) multiple\ elements (crm) debugging (elp) emacs-lisp (find-func) functions (find-func) variables (find-func) compiler (macroexp) macros (macroexp) strings (rx regexp-opt) regexps (rx regexp-opt) parsing (smie) indentation (smie) utility (unsafep testcover) safety (unsafep testcover) burma (emacs) cham (emacs) chinese (emacs) cyrillic (emacs) czech (emacs) ethiopic (emacs) european (emacs) greek (emacs) korean (emacs) hanja (emacs) hebrew (emacs) indian (emacs) devanagari (emacs) khmer (emacs) lao (emacs) romanian (emacs) sinhala (emacs) slovak (emacs) tai\ viet (emacs) thai (emacs) word\ break (emacs) emacs (emacs) tibetan (emacs) vietnamese (emacs) relaxng (nxml) ada (ada-mode) project\ file (ada-mode) xref (ada-mode) antlr (antlr-mode) code\ generator (antlr-mode) awk (cc-mode) cc-mode (cc-mode) oop (cc-guess) perl (cperl-mode) dcl (dcl-mode) ebnf (ebnf2ps) c++ (hideshow ebrowse) fortran (org fortran f90) f90 (f90) java (conf-mode hideshow) comments (hideshow) blocks (hideshow) hiding (hideshow) javascript (js) g++ (mantemp) templates (mantemp) metafont (meta-mode) metapost (meta-mode) knuth (mixal-mode) mix (mixal-mode) mixal (mixal-mode) asm (mixal-mode) mixvm (mixal-mode) the\ art\ of\ computer\ programming (mixal-mode) prolog (prolog) major (prolog) mode (prolog) sicstus (prolog) swi (prolog) mercury (prolog) ruby (ruby-mode) tcl (tcl) modes (tcl) vera (vera-mode) vhdl (vhdl-mode) mode-line (which-func) imenu (which-func) cancel-lock (gnus) hmac (hmac-md5 hmac-def gnus) sha1 (gnus) rfc2104 (hmac-md5 hmac-def gnus) web (gnus) icalendar (gnus) org (org gnus) xpm (gnus) annotation (gnus) glyph (gnus) registry (gnus) synchronization (gnus) nntp (gnus) nnrss (gnus) network (network-stream net-utils dns gnus) external-body (gnus) partial (gnus) shar (gnus) forward (gnus) gnatsweb (gnus) gnus (gnus) s/mime (gnus) mml (gnus) searching (mairix gnus) ir (gnus) rss (newsticker gnus) hz (gnus) hz+ (gnus) smime (gnus) x\.509 (gnus) pem (gnus) openssl (gnus) gmane (gnus) report (gnus) bogofilter (gnus) bbdb (gnus) dspam (gnus) dig (dig gnus) whitelist (gnus) blacklist (gnus) spamassassin (gnus) bsfilter (gnus) ifile (gnus) stat (gnus) crm114 (gnus) spamoracle (gnus) tls (tls gnutls gnus) ssl (tls gnutls gnus) gnutls (tls gnus) yenc (gnus) literate\ programming (org) reproducible\ research (org) r (org) statistics (org) comint (org) js (org) accounting (org) babel\ language (org) maxima (org) scheme (ede org) interactive\ shell (org) shen (org) task (org) capture (org) org-mode (org) erc (org) link (org) tables (org) plotting (org) emacsclient (org) markdown (org) hypertext (browse-url) dns (dns-mode dig) bind (dig) encryption (gnutls) mh-e (goto-addr) www (webjump goto-addr) hmac-md5 (hmac-md5) md5 (hmac-md5) keyed-md5 (hmac-md5) cram-md5 (sasl hmac-md5) atom (newsticker) ntlm (sasl ntlm) sasl (sasl ntlm) digest-md5 (sasl) soap (soap-client) web-services (soap-client) firewalls (socks) bib (refer refbib bib-mode) bibtex (bibtex) conf (conf-mode) ini (conf-mode) windows (conf-mode) master (dns-mode) zone (dns-mode) soa (dns-mode) memory (remember) pim (remember) text (tildify) sgml (tildify) project (ede semantic) make (ede semantic) codegeneration (srecode))))

(provide 'finder-inf)
;; Local Variables:
