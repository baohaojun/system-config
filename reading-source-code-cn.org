...
** 强大的社区和库的支持——内功
   ...
   因为它的语言自带的标准库提供了很强大的功能。Perl的背后有cpan（相信我，
   里面真的简直什么都有）。TeX背后有ctan。Emacs的背后有EmacsWiki。
...
3. 退出编辑器，回到命令行，运行 make，查开结果，如果出错，回第2步。
...
*** 文档
   ...
   Emacs里打开看，并且比起在终端上用man(1)命令打开，在Emacs里你可以调用
   更丰富的浏览、搜索功能。
...
这种用法我用的最多的是org-mode的发布功能，比如的我github page，我用
org-mode写完一篇文章之后，可能忘了把它发布为html，所以我在git push的时
候可以加个hook，自动检查一下，相应的.html文件是不是没有更新，如果没有的
话，就逆向调用Emacs一下，自动完成.html的发布（要不然的话还手动打开
Emacs，再打开这个.org文件，再手动发布，就太烦了）。

所以这就是我喜欢用Emacs的原因，内外兼修，无限可能。下面我们开始讲怎样调
用一个个具体的外部程序来把Emacs打造成强大的代码阅读工具。
...
从compilation-mode继承下来的。这种输出有一个模式，那就是
 ~文件:行号: 内容~ 。所以Emacs很容易parse出应该跳转到哪个文件的哪一行上。

跳转到下一个匹配行的Emacs函数是 ~next-error~ （由此可见其与
compilation-mode的联系，把grep的匹配也称之为一个error，软件重用是好的，
但真的会造成困惑吧）。
...
希望第二个parse_state出现在最前面，如果用户在看iptables相关的代码，那么
很可能需要把4、5、6排在前面。
...
在读这个文件代码的时候，我们想查一下 ~parse_state~ ，查出来有一条
~/home/bhj/src/android/system/core/init/parser.c~ 里面包含有这个定义，
...
我的imenu的用法当然跟source insight有点不一样，但精神上是类似的。首先，
...
(图中只显示了 ~macro~ ）。Emacs加一分！~anything~ 的好处在于你还可以接
着打一个空格，再打一个正则表达式，继续过滤出你真正想要的定义。
...
有一个函数定义A_func，150行有一个函数B_func，而readlink出现在了125行，
...
我曾经也用过这两个工具，但目前已经淘汰了前者，后者则只用它调用
ctags-exuberant的plugin和模仿cscope格式的输出功能（后来把这种输出格式又
改成grep-mode了，但是你还是可以在我的.emacs里看到cscope.el里实现的定义
查询相关的前进、后退功能）

* 查找局部变量的定义

呵呵，这个也是可以实现的，给大家当脑筋急转弯吧。
