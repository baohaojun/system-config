      ...
      (let* ((resolve
             ...
             (class (cond
                     ...
                     ((let ((case-fold-search nil))
                      ...
                      (mapconcat 'identity (butlast comp) "."))
                     (t resolve)))
            (hierarchy (shell-command-to-string
                        (cond
                         ((string= "darwin" system-type)
                          (format "ajoke-get-hierarchy.pl %s -v 2>&1|grep -v 'Unknown option:' |perl -npe 's/^\\s+//'|sort -u" class))
                         (t
                          (format "ajoke-get-hierarchy.pl %s -v|grep '('|perl -npe 's/^\\s+//'|sort -u" class)))))
             (methods (split-string hierarchy "\n")))
