      ...
      (let* ((resolve
             ...
             (class (cond
                     ...
                     ((let ((case-fold-search nil))
                      ...
                      (mapconcat 'identity (butlast comp) "."))
                     (t resolve)))
;;debug             (message-debug (message "|pwd,%s|gtagsroot,%s|gtagsdbpath,%s|gtagslibpath,%s|gtagsconf,%s|gtagslabel,%s|makeobjdirprefix,%s|global-p,%s|" (getenv "PWD") (getenv "GTAGSROOT") (getenv "GTAGSDBPATH") (getenv "GTAGSLIBPATH") (getenv "GTAGSCONF")  (getenv "GTAGSLABEL")  (getenv "MAKEOBJDIRPREFIX") (shell-command-to-string "global -p") ))
            ;; (hierarchy (shell-command-to-string (format "ajoke-get-hierarchy.pl %s -v|grep '('|perl -npe 's/^\\s+//'|sort -u" class)))
            (hierarchy (shell-command-to-string (format "ajoke-get-hierarchy.pl %s -v 2>&1|grep -v 'Unknown option:' |perl -npe 's/^\\s+//'|sort -u" class)))
             (methods (split-string hierarchy "\n")))
