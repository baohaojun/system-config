...
(setq package--builtins '(
  ...
  (calculator . [nil nil "a calculator for Emacs"])
  ...
  (cfengine . [(1 4) nil "mode for editing Cfengine files"])
  (character-fold . [nil nil "match unicode to similar ASCII"])
  ...
  (cl-generic . [(1 0) nil "CLOS-style generic functions for Elisp"])
  ...
  (cursor-sensor . [nil nil "React to cursor movement"])
  ...
  (dom . [nil nil "XML/HTML (etc.) DOM manipulation and searching functions"])
  (double . [nil nil "support for keyboard remapping with double clicking"])
  (dunnet . [(2 2) nil "text adventure for Emacs"])
  ...
  (inline . [nil nil "Define functions by their inliner"])
  ...
  (let-alist . [(1 0 4) nil "Easily let-bind values of an assoc-list by their names"])
  ...
  (map . [(1 0) nil "Map manipulation functions"])
  ...
  (newsticker . [nil nil "A Newsticker for Emacs."])
  ...
  (nsm . [nil nil "Network Security Manager"])
  (ntlm . [(2 0 0) nil "NTLM (NT LanManager) authentication support"])
  ...
  (package . [(1 1 0) nil "Simple package system for Emacs"])
  ...
  (pinentry . [(0 1) nil "GnuPG Pinentry server implementation"])
  ...
  (project . [nil nil "Operations on the current project"])
  ...
  (python . [(0 25 1) nil "Python's flying circus support for Emacs"])
  ...
  (rfc2104 . [nil nil "RFC2104 Hashed Message Authentication Codes"])
  ...
  (seq . [(2 3) nil "Sequence manipulation functions"])
  ...
  (soap-client . [(3 0 2) nil "Access SOAP web services"])
  ...
  (sql . [(3 5) nil "specialized comint.el for SQL interpreters"])
  ...
  (thunk . [(1 0) nil "Lazy form evaluation"])
  (tildify . [(4 6 1) nil "adding hard spaces into texts"])
  ...
  (tramp . [(2 2 13 25 1) nil "Transparent Remote Access, Multiple Protocol"])
  ...
  (xref . [nil nil "Cross-referencing commands"])
  (xscheme . [nil nil "run MIT Scheme under Emacs"])
  (xt-mouse . [nil nil "support the mouse when emacs run in an xterm"])
  (xwidget . [nil nil "api functions for xwidgets"])
  (zeroconf . [nil nil "Service browser using Avahi."])
  (zone . [nil nil "idle display hacks"])
))

(setq finder-keywords-hash
      #s(hash-table size 487 test eq rehash-size 1.5 rehash-threshold 0.8 data (abbrev (skeleton pcomplete emacs icomplete hippie-exp expand edmacro dabbrev completion autoarg) convenience (diff-mode compare-w table picture emacs makeinfo flyspell erc keypad cua-base map calc winner windmove vcursor tmm scroll-all ruler-mode repeat pcomplete minibuf-eldef mb-depth linum kmacro json imenu ido ibuffer hippie-exp frameset follow filesets filecache ffap ebuff-menu desktop delsel dabbrev completion chistory calculator bs autorevert autoinsert align) languages (sgml-mode xscheme vhdl-mode verilog-mode vera-mode tcl sql simula sh-script scheme ruby-mode python ps-mode perl-mode pascal opascal octave modula2 mixal-mode meta-mode m4-mode ld-script js idlwave icon fortran flymake f90 executable emacs dcl-mode cwarn cperl-mode cfengine cc-mode cc-guess bat-mode autoconf asm-mode antlr-mode ada-mode nxml erc smie tempo skeleton paren info-look allout align) lisp (xscheme scheme inf-lisp hideshow emacs unsafep trace testcover tabulated-list smie shadow re-builder pp macroexp let-alist ewoc ert elp elint eieio eieio-core edebug debug checkdoc check-declare benchmark inversion cedet strokes profiler isearchb ielm color cmuscheme align) outlines (hideshow hideif org reveal outline foldout elide-head allout allout-widgets) wp (underline two-column tildify emacs table sgml-mode refill picture page-ext nroff-mode ispell enriched ebnf2ps handwrite org nxml whitespace ps-print printing allout) pgp (gnus epg epa emacs allout) gnupg (pinentry gnus epg epa emacs allout) comm (url sgml-mode dns-mode sql zeroconf webjump tramp tls telnet socks soap-client secrets rlogin rcirc ntlm net-utils ldap gnutls eudc dns dig dbus ange-ftp erc bindat talk shadowfile notifications master kermit ansi-color) processes (url sql inf-lisp idlwave grep compile tramp eshell erc bindat term shell server proced pcomplete emacs emacs-lock dirtrack cmuscheme ansi-color) terminals (tramp xt-mouse vt-control ansi-color) services (ansi-color) help (emacs woman novice man makesum informat info info-look icomplete help-at-pt finder ehelp) files (compare-w url po wdired view emacs shadowfile recentf locate ffap doc-view arc-mode) archives (arc-mode) ms-dos (arc-mode) editing (hideshow dcl-mode arc-mode) major-mode (dcl-mode arc-mode) extensions (cookie1 gnus tq tabulated-list rx ring regi regexp-opt emacs let-alist ewoc eldoc cl cl-lib benchmark avl-tree calc type-break tree-widget thingatpt tempo strokes skeleton ido emacs-lock electric ehelp array) emulations (edt cua-base emacs hl-line delsel autoarg) mouse (artist goto-addr browse-url xt-mouse emacs thingatpt t-mouse strokes msb mouse-drag mouse-copy ffap avoid) hardware (zeroconf dbus vt100-led emacs flow-ctrl battery) internal (emacs warnings syntax smie disass select novice loadhist gs delim-col) bookmarks (saveplace bookmark) placeholders (saveplace bookmark) annotations (bookmark) tools (vc smerge-mode log-view emerge ediff diff cvs-status add-log which-func make-mode hideshow gud grep glasses gdb-mi flymake etags ebrowse cpp compile bug-reference asm-mode reporter mail-utils trace re-builder package emacs ert elp eieio edebug debug copyright check-declare semantic time-stamp tempo speedbar skeleton sb-image pcomplete imenu format-spec find-file ezimage elide-head dframe calculator) i18n (po emacs gnus double descr-text) data (url remember page-ext org snmp-mode gnus bindat timeclock xml whitespace jka-compr emacs hexl hex-util cdl) matching (regi re-builder thingatpt soundex misearch emacs hi-lock find-file ffap character-fold) faces (enriched m4-mode ld-script cwarn cpp gnus erc emacs paren hl-line hilit-chg hi-lock face-remap descr-text color) color (color) hex (color) rgb (htmlfontify color) hsv (color) hsl (color) cie-lab (color) background (color) mule (emacs ps-print) multilingual (emacs descr-text) character\ composition (emacs) expand (dabbrev) completion (crm dabbrev) unicode (emacs descr-text) file (dns-mode speedbar sb-image ezimage dframe) tags (ebrowse semantic ede speedbar sb-image ezimage dframe) dired (wdired emacs) window (emacs windmove follow) drag (emacs) drop (emacs) pdf (doc-view) ps (doc-view) dvi (doc-view) xml (nxml xml dom) html (shr shr-color eww gnus htmlfontify dom) font (ps-print emacs) system-font (emacs) tool-bar-style (emacs) unix (emerge ediff diff ispell sh-script make-mode gud gdb-mi executable cc-mode telnet rlogin woman tar-mode sort proced emacs lpr locate find-lisp find-dired) mail (fortune rfc2104 mairix imap goto-addr mh-e unrmail undigest uce supercite smtpmail sendmail rmail rfc822 rfc2368 reporter mspools metamail mail-utils mailclient mailalias mailabbrev hashcash footnote feedmail emacs gnus time-date epa) message (feedmail gnus epa) face\ remapping (face-remap) display (hi-lock follow face-remap) user\ commands (face-remap) hypermedia (url sgml-mode css-mode org nxml soap-client quickurl browse-url emacs htmlfontify ffap) filesets (filesets) c (hideshow hideif flymake cwarn cpp cmacexp cc-mode cc-guess emacs find-file) folding (foldout) minor-mode (hi-lock follow) frames (winner emacs talk hl-line) generic (emacs) comment (emacs) font-lock (emacs) colour (htmlfontify) markup (htmlfontify) etags (htmlfontify) buffer (msb ibuffer) multimedia (metamail gnus thumbs mpc emacs image-file image-dired iimage) docs (texnfo-upd texinfo texinfmt makeinfo lisp-mnt checkdoc info-xref) keyboard (keypad cua-base kmacro) md4 (md4) utilities (midnight) menu (erc msb) uncomment (emacs) desktop (notifications) notifications (notifications) obarray (emacs) functions (find-func emacs) password (secrets password-cache) cache (password-cache) passphrase (secrets password-cache) key (password-cache) print (handwrite ps-print printing) postscript (ps-mode ebnf2ps handwrite gnus ps-print printing) bdf (ps-print) multibyte (ps-print) vi (repeat) repeat (repeat) minibuffer (crm savehist emacs) scroll (scroll-all) lock (scroll-all) spreadsheet (testcover ses) dijkstra (ses) gpm (t-mouse) linux (t-mouse) news (newsticker netrc uudecode supercite metamail mail-utils footnote binhex gnus time-date timezone) timers (type-break) virtual\ cursor (vcursor) environment (wdired) renaming (wdired) movement (windmove) calendar (org gnus todo-mode timeclock calendar icalendar diary-lib) holidays (calendar) util (time-date parse-time) todo (remember todo-mode) oo (eieio eieio-core chart inversion cedet) project (semantic ede) make (semantic ede) syntax (emacs semantic mode-local) codegeneration (srecode) scheme (org ede) vc (vc smerge-mode pcvs log-view log-edit emerge ediff diff diff-mode cvs-status compare-w add-log url ede) maint (texnfo-upd texinfo texinfmt reporter emacs edebug debug copyright checkdoc check-declare) data\ structures (avl-tree) avl (avl-tree) tree (cvs-status avl-tree) chart (chart) graph (chart) multiple\ elements (crm) lisp\ or\ maybe\ eric\'s\ implementation\ of\ emacs\ interpreted\ objects (eieio) debugging (elp) emacs-lisp (find-func) variables (find-func) elisp (emacs) compiler (macroexp) macros (macroexp) map (map) hash-table (map) alist (map) array (map) strings (rx regexp-opt) regexps (rx regexp-opt) sequences (thunk seq) parsing (smie) indentation (smie) utility (unsafep testcover) safety (unsafep testcover) cua (cua-base) mark (cua-base) emulations\ \	\ a\ vi\ plan\ for\ emacs\ rescue (viper) and\ a\ venomous\ vi\ peril\.\ \	\ viper\ is\ also\ a\ package\ for\ emacs\ rebels\. (viper) irc (org erc) chat (erc) client (erc) internet (erc) button (erc) url (vc url erc) regexp (nxml erc) logging (erc) timestamp (erc) cancel-lock (gnus) hmac (hmac-md5 hmac-def gnus) sha1 (gnus) rfc2104 (hmac-md5 hmac-def gnus) web (gnus) icalendar (gnus) org (org gnus) xpm (gnus) annotation (gnus) glyph (gnus) registry (gnus) synchronization (gnus) nntp (gnus) nnrss (gnus) network (nsm network-stream net-utils dns gnus) external-body (gnus) partial (gnus) uudecode (uudecode gnus) binhex (binhex gnus) shar (gnus) forward (gnus) gnatsweb (gnus) gnus (gnus) mime (metamail gnus) s/mime (gnus) mml (gnus) searching (mairix gnus) ir (gnus) rss (newsticker gnus) hz (gnus) hz+ (gnus) smime (gnus) x\.509 (gnus) pem (gnus) openssl (gnus) spam (mh-e rmail gnus) gmane (gnus) report (gnus) bogofilter (gnus) bbdb (gnus) dspam (gnus) dig (dig gnus) whitelist (gnus) blacklist (gnus) hashcash (hashcash gnus) spamassassin (gnus) bsfilter (gnus) ifile (gnus) stat (gnus) crm114 (gnus) spamoracle (gnus) tls (tls gnutls gnus) ssl (tls gnutls gnus) gnutls (tls gnus) yenc (gnus) ccl (emacs) character\ set (emacs) coding-system (emacs) multibyte\ character (emacs) category (emacs) fontset (emacs) isearch (emacs) tex (tildify texnfo-upd texinfo texinfmt tex-mode reftex refbib bibtex bibtex-style meta-mode org emacs) iso (emacs) latin (emacs) japanese (emacs) kinsoku (emacs) coding\ system (emacs) latex (bibtex emacs) charset (emacs) diagnosis (emacs) input\ method (emacs) quail (emacs) tit (emacs) cxterm (emacs) normalization (emacs) burma (emacs) cham (emacs) chinese (emacs) cyrillic (emacs) czech (emacs) ethiopic (emacs) european (emacs) greek (emacs) korean (emacs) hanja (emacs) hebrew (emacs) indian (emacs) devanagari (emacs) khmer (emacs) lao (emacs) romanian (emacs) sinhala (emacs) slovak (emacs) tai\ viet (emacs) thai (emacs) word\ break (emacs) emacs (emacs) tibetan (emacs) vietnamese (emacs) email (rmail feedmail) queue (feedmail) sendmail (feedmail) spray (feedmail) smtp (feedmail) draft (feedmail) history (mail-utils) filter (rmail) rmail (rmail) uce (uce) unsolicited\ commercial\ email (uce) hypertext (browse-url) dns (dns-mode dig) bind (dig) encryption (nsm gnutls) mh-e (goto-addr) www (webjump goto-addr) hmac-md5 (hmac-md5) md5 (hmac-md5) keyed-md5 (hmac-md5) cram-md5 (sasl hmac-md5) atom (newsticker) security (nsm) ntlm (sasl ntlm) sasl (sasl ntlm) digest-md5 (sasl) soap (soap-client) web-services (soap-client) firewalls (socks) relaxng (nxml) literate\ programming (org) reproducible\ research (org) r (org) statistics (org) comint (org) fortran (fortran f90 org) js (org) accounting (org) babel\ language (org) maxima (org) interactive\ shell (org) shen (org) task (org) capture (org) org-mode (org) erc (org) link (org) tables (org) plotting (org) emacsclient (org) markdown (org) games (zone tetris studly spook solitaire snake pong mpuz morse life hanoi gomoku gametree gamegrid fortune dunnet doctor dissociate decipher cookie1 bubbles blackbox animate 5x5) puzzles (5x5) utils (fortune) cursive\ writing (handwrite) ada (ada-mode) project\ file (ada-mode) xref (ada-mode) antlr (antlr-mode) code\ generator (antlr-mode) awk (cc-mode) cc-mode (cc-mode) oop (cc-guess) perl (cperl-mode) dcl (dcl-mode) ebnf (ebnf2ps) c++ (hideshow ebrowse) f90 (f90) java (conf-mode hideshow) comments (hideshow) blocks (hideshow) hiding (hideshow) javascript (js) g++ (mantemp) templates (mantemp) metafont (meta-mode) metapost (meta-mode) knuth (mixal-mode) mix (mixal-mode) mixal (mixal-mode) asm (mixal-mode) mixvm (mixal-mode) the\ art\ of\ computer\ programming (mixal-mode) prolog (prolog) major (prolog) mode (prolog) sicstus (prolog) swi (prolog) mercury (prolog) ruby (ruby-mode) tcl (tcl) modes (tcl) vera (vera-mode) vhdl (vhdl-mode) mode-line (which-func) imenu (which-func) bib (refer refbib bib-mode) bibtex (bibtex) conf (conf-mode) ini (conf-mode) windows (conf-mode) master (dns-mode) zone (dns-mode) soa (dns-mode) memory (remember) pim (remember) text (tildify) sgml (tildify) pcl-cvs (pcvs log-edit cvs-status) cvs (smerge-mode pcvs log-edit cvs-status) status (cvs-status) patch (diff-mode) diff (diff-mode) comparing (ediff) merging (ediff) patching (ediff) commit (log-edit) log (log-edit) release\ management (pcvs) revision\ control (smerge-mode) merge (smerge-mode) diff3 (smerge-mode) conflict (smerge-mode))))

(provide 'finder-inf)
;; Local Variables:
