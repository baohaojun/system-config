...
0. Put ajoke scripts into you PATH:
   
   #+BEGIN_SRC sh
   export export PATH=$ajoke_dir/bin:$PATH
   #+END_SRC

1. Install 3 cpan modules: String::Approx, String::ShellQuote,
   Text::Glob (as debian packages, they are named
   libstring-approx-perl, libstring-shellquote-perl and
   libtext-glob-perl)

2. Compile and install the patched version of Gnu Global:
...
4. Copy the .globalrc and .ctags into your =$HOME=:
...
5. Create Gnu Global tags for JDK (taking my openjdk installation as
   e.g.):
...
6. In your Java project, create Gnu Global tags again, and add the JDK
   or Android as fallback:
...
7. Require $ajoke_dir/etc/elisp/ajoke.el in your .emacs, and start
   laughing:-)

You can refer to the test driver script if there are any problems. In
the mean time, I will try to make =ajoke-test-drive= to support more
systems.
