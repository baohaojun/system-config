(defun synthesize-multiple-events (&rest events)
  (mapc (lambda (e)
        ...
        events))
(bind-keys global-keymap "Next" '(if (string-match (bhj-window-class (input-focus)) "Qpdfview")
                                     (synthesize-multiple-events "SPC")
                                   (synthesize-multiple-events "Next")))
(bind-keys global-keymap "Prior" '(if (string-match (bhj-window-class (input-focus)) "Qpdfview")
