...
if ! [[ -f "$PROJROOT"/package.list ]]; then 
/vob/ilj_release/devkit/pck/pck.sh apt-cache dumpavail "$PROJROOT" |grep Package:|sed -e 's/Package: //g'|sort -u >"$PROJROOT"/package.list; 
Debug "after apt-cache dumpavail"
fi

/vob/ilj_release/devkit/pck/pck.sh getdirsize "$PROJROOT"/rootfs

vi "$PROJROOT"/package.list

grep -v "#" "$PROJROOT"/package.list |(subfile=$TMPDIR/already.txt ~a22242/bin/file_subtract.pl) > "$PROJROOT"/package.list.tmp
