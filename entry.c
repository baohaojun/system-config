...
extern boolean statementEnded(const char *line, const char *chars)
{
    int i;
    for (i = 0; chars[i]; i++) {
        if (strchr(line, chars[i]))
            return TRUE;
    }
...
static int writeXrefEntry (const tagEntryInfo *const tag)
{
        ...
        vString *protoStr = NULL;
        if (tag->endProtoChars && !statementEnded(line, tag->endProtoChars)) {
            protoStr = vStringNew();
            readStatement(protoStr, tag->filePosition, tag->endProtoChars);
            vStringCompressWhiteSpace(protoStr);
            line = vStringValue(protoStr);
        }
