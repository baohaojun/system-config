...
我新安装一台机器后，我可以通过一个简单的 ~git clone --recursive http://github.com/baohaojun/system-config~ 和额外的几条脚本，把
我的个人环境快速重建出来。
...
~$HOME/system-config/~ 下之后，只要记得跑这两条命令就搞定了：
...
猜到 [[../29/why-symlink-my-config.org][是什么]] 吗？第二条则是做一些.deb包的安装、编译工作。
