...
#+BEGIN_SRC perl
  ...
  sub jkd_c(@) { # create issue
      ...
      my $issue_type_json;
      if (not $issue_type or not $issue_type =~ m/^\d+$/) {
          $issue_type_json = from_json(capturex("jkd", "select-issuetype", "-p", "${project}", "--json"));
      ...
      if ($fields_json) {
          $fields_json = capturex("jkd", "customfield-json-names2ids", "-n", decode_utf8("$fields_json"), "-f", decode_utf8(to_json($issue_type_json->{fields})));
      ...
      for (my $try = 0; $try < 3; $try++) {
          ...
          say "POST \@${try} response code:" . $response->code, "result: ", $response->content;
