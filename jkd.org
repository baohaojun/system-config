...
#+BEGIN_SRC perl
  ...
  sub jkd_c(@) { # create issue
      ...
      if (not $issue_type) {
          ...
          my $issue_type_name = select_args("-p", "Which type of issue do you want?", "-O", "select-issue-type", sort {$a cmp $b} keys %jkd_project_issue_types_name_id_map);
          $issue_type = $jkd_project_issue_types_name_id_map{$issue_type_name};
      }

      my $issue_fields_obj = jkd_get_issue_fields("-p", "$project", "-i", "$issue_type");

      my %required_fields;
      for my $project (@{$issue_fields_obj->{projects}}) {
          for my $issue_type (@{$project->{issuetypes}}) {
              for my $field_name (keys %{$issue_type->{fields}}) {
                  if ($issue_type->{fields}{$field_name}{required}) {
                      if ($field_name eq 'project' || $field_name eq 'issuetype') {
                          next;
                      }
                      if ($issue_type->{fields}{$field_name}{schema}{type} eq 'string') {
                          my @command = (
                              "ask-for-input-with-emacs", "-p", "Please input the $field_name."
                              );
                          my $command = join(" ", shell_quote(@command));
                          $required_fields{$field_name} = qx($command);
                      }
                  }
