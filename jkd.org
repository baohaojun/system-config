...
This is simple using Perl's “reflection”.
  ...
  #+BEGIN_SRC perl
    ...
    if (not defined &$sub_command) {
        (my $lib_command = $sub_command) =~ s,^jkd_,jkd/,;

        $lib_command = "$ENV{scm_common_libdir}/$lib_command";

        if (-x $lib_command) {
            exit system("$lib_command", @ARGV);
        }
...
#+BEGIN_SRC perl
  ...
  sub jkd_print_issue(@) {
      ## start code-generator "^\\s *#\\s *"
      # generate-getopt -s perl i:issue f:fields-to-print='()' @json @for-clone @pretty-names
      ## end code-generator
      ## start generated code
      use Getopt::Long;
      ...
      my $handler_help = sub {
          ...
          print "\n";
          printf "%6s", '';
          printf "%-24s", '--[no]pretty-names';
          if (length('--[no]pretty-names') > 24 and length() > 0) {
              print "\n";
              printf "%30s", "";
          }
          printf "%s", ;
