...
#+BEGIN_SRC perl
  ...
  sub jkd_e(@) {
      ...
      for my $try (1..3) {
          ...
          say "PUT \@${try} response code:" . $response->code, "result: ", $response->content;
  ...
  sub jkd_c(@) { # create issue
      ...
      my $issue_type_json;
      if (not $issue_type or not $issue_type =~ m/^\d+$/) {
          $issue_type_json = from_json(capturex("jkd", "select-issuetype", "-i", "$issue_type", "-p", "${project}", "--json"));
      ...
      if ($fields_json) {
          $fields_json = capturex("jkd", "customfield-json-names2ids", "-n", ("$fields_json"), "-f", decode_utf8(to_json($issue_type_json->{fields})));

          my $required_fields = decode_json (encode_utf8 $fields_json);
      ...
      for (my $try = 0; $try < 3; $try++) {
          ...
          say STDERR "POST \@${try} response code:" . $response->code, "result: ", $response->content;
  ...
  #+BEGIN_SRC perl
    ...
    sub jkd_tri(@) {
        ...
        if ($transition !~ m/^\d+$/ or $fields_json ne "{}") {
            ...
            $transition_json_obj = decode_json (encode_utf8 $response->content);
        }
