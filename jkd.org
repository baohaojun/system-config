...
#+BEGIN_SRC perl
          ...
          for my $it (@{$project->{issuetypes}}) {
              ...
              for my $field_key (@fields_to_edit) {
                  if ($it->{fields}{$field_key}{required}) {
                      ...
                      say STDERR "fild value is ", encode_utf8($edit_issue_json_obj->{fields}{$field_key} || "");
  ...
  sub jkd_e(@) {
      ...
      work_with_all_fields($issue_project, $issue_type_id, %edited_fields, {edit_issue_json_obj => $json_issue});
  ...
  sub jkd_c(@) { # create issue
      ...
      work_with_all_fields($project, $issue_type_id, %required_fields, {"print-schemes" => ${print_schemes}});
