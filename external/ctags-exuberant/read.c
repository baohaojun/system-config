...
extern char *readStatement (vString *const vLine, fpos_t location, const char *endChars)
{
        ...
        vString* tmp_line = vStringNew();
        vStringClear(vLine);
        while (vStringLength(vLine) < 1024) {
                result = readLine (tmp_line, File.fp);
                if (!result)
                        break;

                vStringCatS(vLine, result);
                if (statementEnded(result, endChars)) {
                        result = vLine->buffer;
                        break;
                }
        }
        vStringDelete(tmp_line);
        fsetpos (File.fp, &orignalPosition);
        return result;
}
