...
/* Compress multiple spaces into 1 space.
 */
extern void vStringCompressWhiteSpace (vString *const string)
{
        size_t i, j;
        for (i = 0, j = 0; i < string->length && j < string->length ; ++i, ++j) {
                string->buffer [i] = string->buffer [j];
                if (isspace ((int) string->buffer [j])) {
                        string->buffer [i] = ' '; // in case it *was* a \n
                        for (; j < string->length && isspace ((int) string->buffer [j]); ++j)
                                ;
                        --j;
                }
        }
