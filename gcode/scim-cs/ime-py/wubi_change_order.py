...
if re.search("wubi_change_order.py", sys.argv[0]):
    keys = list(wubi.g_quail_map.keys())
    keys.sort()

    print_head()
    for code in keys:
        words = wubi.g_quail_map[code]
        need_fix = False
    ...
    sys.stdout.write("}\n")
elif re.search("wubi_detect_multi.py", sys.argv[0]):
    g_hanzi_code_map = {}

    for code in wubi.g_quail_map.keys():
        if code[0] == 'z':
            continue
        words = wubi.g_quail_map[code]
        for hanzi in words:
...
elif re.search("wubi_delete_code.py", sys.argv[0]):
    keys = list(wubi.g_quail_map.keys())
    keys.sort()
    ...
    for code in keys:
        if code == sys.argv[1]:
            ...
            words = tuple(words)
            wubi.g_quail_map[code] = words
        print_code_words(code, wubi.g_quail_map[code])
