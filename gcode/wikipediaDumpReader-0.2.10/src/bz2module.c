...
"The original bz2 python module was written by:\n\
\n\
...
#endif
	struct currentBlockInfos blockInfos;
} BZ2FileObject;
...
PyDoc_STRVAR(BZ2File_loadBlock__doc__,
"loadBlock(byteOffset, bitOffset, offsetInBlock, len) -> None\n\
\n\
");

char* loadBlock ( BZFILE*, long long, int, unsigned int *);

static PyObject *
BZ2File_loadBlock(BZ2FileObject *self, PyObject *args)
{
	PyObject *ret = NULL;
	...
	if (!PyArg_ParseTuple(args, "LiIi", &byteOffset, &bitOffset, &offset, &len))
		return NULL;

	BZFILE *strm = self->fp;
	outbuf = (char*)loadBlock(strm, byteOffset, bitOffset, &nOut);
	if (PyErr_Occurred()) return NULL;
	if (outbuf == NULL) {
	 PyErr_SetString(PyExc_IOError, "error in seeking - corrupted file ?");
		return NULL;
	}
	if (nOut - offset >= len) 
		v = PyString_FromStringAndSize(&(outbuf[offset]), len);
	else
		v = PyString_FromStringAndSize(&(outbuf[offset]), nOut-offset);
		// user will need to read "len - (nOut - offset)" of next one
	free(outbuf);
	return Py_BuildValue("(Oi)", v, nOut);
//	return v;
}

PyDoc_STRVAR(BZ2File_tellbzblock__doc__,
"tellbzblock() -> int\n\
\n\
Return the beginning offset of the current block.\n");
/*
typedef
   struct {
      FILE*     handle;
...
*/
static PyObject *
BZ2File_tellbzblock(BZ2FileObject *self, PyObject *args)
{
	PyObject *ret = NULL;
...
Util_GetLine(BZ2FileObject *f, int n)
{
	...
	for (;;) {
		...
		if (univ_newline) {
			while (1) {
				f->blockInfos.pos = f->pos;
				My_bzRead(&bzerror, f->fp, &c, 1, &f->blockInfos);
				f->pos++;
		     ...
		     char* buf, size_t n, BZ2FileObject *f)
{
	...
	if (!f->f_univ_newline)
		return My_bzRead(bzerror, stream, buf, n, &f->blockInfos);
	...
	while (n) {
		...
		nread = My_bzRead(bzerror, stream, dst, n, &f->blockInfos);
		assert(nread <= n);
...
static PyMethodDef BZ2File_methods[] = {
	...
	{"tellbzblock", (PyCFunction)BZ2File_tellbzblock, METH_NOARGS, BZ2File_tellbzblock__doc__},
	{"loadBlock", (PyCFunction)BZ2File_loadBlock, METH_VARARGS, BZ2File_loadBlock__doc__},
	{NULL,		NULL}		/* sentinel */
};
...
BZ2File_init(BZ2FileObject *self, PyObject *args, PyObject *kwargs)
{
	...
	self->blockInfos.currentBlockByte = 0;
	self->blockInfos.currentBlockBitOffset = 0;
	return 0;
...
Warning : this is a modified version, not the original one.\n\
");
