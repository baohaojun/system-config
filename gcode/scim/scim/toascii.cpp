...
/**********************************************************************/
UINT PASCAL
GBProcessKey (			// this key will cause the IME go to what state
	       WORD wCharCode, LPPRIVCONTEXT lpImcP)
{
  ...
  if ((wCharCode >= TEXT ('0') && wCharCode <= TEXT ('9'))
      ...
      else if (!lpImcP->bSeq[1])
	{
	  ...
	  if (lpImcP->bSeq[0] >= TEXT ('0') && lpImcP->bSeq[0] <= TEXT ('9'))
	    {			//Area
		  ...
		  || (lpImcP->bSeq[0] == TEXT ('9')
		      && wCharCode >= TEXT ('5')) || (wCharCode >= TEXT ('a')
						      && wCharCode <=
						      TEXT ('f'))
		  || (wCharCode == TEXT ('?')))
		{
	  ...
	  if (lpImcP->bSeq[0] >= TEXT ('a') && lpImcP->bSeq[0] <= TEXT ('f'))
	    {			//GB
...
/**********************************************************************/
UINT PASCAL
UnicodeProcessKey (		// this key will cause the IME go to what state
		    WORD wCharCode, LPPRIVCONTEXT lpImcP)
{
  if (!lpImcP)
    {
...
/**********************************************************************/
UINT PASCAL
XGBProcessKey (			// this key will cause the IME go to what state
		WORD wCharCode, LPPRIVCONTEXT lpImcP)
{
  if (!lpImcP)
    {
...
/**********************************************************************/
UINT PASCAL
ProcessKey (			// this key will cause the IME go to what state
	     WORD wCharCode,
	     UINT uVirtKey,
	     UINT uScanCode,
	     LPBYTE lpbKeyState, LPINPUTCONTEXT lpIMC, LPPRIVCONTEXT lpImcP)
{
...
/**********************************************************************/
BOOL WINAPI
ImeProcessKey (			// if this key is need by IME?
		HIMC hIMC,
		UINT uVirtKey, LPARAM lParam, CONST LPBYTE lpbKeyState)
{
  ...
  iRet =
    ProcessKey ((WORD) szAscii[0], uVirtKey, HIWORD (lParam), lpbKeyState,
		lpIMC, lpImcP);
  if (iRet == CST_INVALID)
    {
      fRet = FALSE;
...
UINT PASCAL
TranslateToAscii (		// translate the key to WM_CHAR
		   // as keyboard driver
		   UINT uVirtKey,
		   UINT uScanCode, LPTRANSMSGLIST lpTransBuf, WORD wCharCode)
{
  LPTRANSMSG lpTransMsg;
	      ...
	      LPTRANSMSGLIST lpTransBuf, UINT fuState, HIMC hIMC)
{
	  ...
	  if (wCharCode == TEXT ('.'))
	    {
	      ...
	      uNumMsg =
		TranslateSymbolChar (lpTransBuf, wSymbolCharCode, FALSE);
	    }
	  else if (wCharCode == TEXT (','))
	    {
  ...
  else if (iRet == CST_INPUT && lpIMC->fdwConversion & IME_CMODE_CHARCODE)
    {
      uNumMsg = TranslateToAscii (uVirtKey, uScanCode, lpTransBuf, wCharCode);
    }
  // CST_INPUT 
