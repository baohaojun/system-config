...
/**********************************************************************/
WORD PASCAL
UnicodeEngine (LPPRIVCONTEXT lpImcP)
{
...
/**********************************************************************/
WORD PASCAL
XGBEngine (LPPRIVCONTEXT lpImcP)
{
  WORD wCode;
...
/**********************************************************************/
WORD PASCAL
XGBSpcEng (LPPRIVCONTEXT lpImcP)
{
  WORD wCode;
  ...
  if (lpImcP->bSeq[3] || (lpImcP->bSeq[2] == TEXT ('?')))
    {
      ...
      if (lpImcP->bSeq[0] >= TEXT ('0') && lpImcP->bSeq[0] <= TEXT ('9'))
	{			//Area mode
	  if (lpImcP->bSeq[2] == TEXT ('?'))
	    {
      ...
      else if (lpImcP->bSeq[0] >= TEXT ('a') && lpImcP->bSeq[0] <= TEXT ('f'))
	{			//GB mode
...
/**********************************************************************/
WORD PASCAL
AsciiToGB (LPPRIVCONTEXT lpImcP)
{
  WORD GBCode;
  ...
  GBCode = GBCode * 256;
  GBCode =
    (CharToHex (lpImcP->bSeq[0]) << 4) + CharToHex (lpImcP->bSeq[1]) + GBCode;
...
/**********************************************************************/
WORD PASCAL
AsciiToArea (LPPRIVCONTEXT lpImcP)
{
  WORD AreaCode;
  AreaCode =
    (CharToHex (lpImcP->bSeq[2]) * 10) + CharToHex (lpImcP->bSeq[3]) + 0xa0;
	  ...
	  for (i = 0; i < IME_UNICODE_MAXCAND; i++, wCode++)
	    {
	      ...
	      *(LPTSTR) ((LPBYTE) lpCandList +
			 lpCandList->dwOffset[lpCandList->dwCount]) = wCode;
	      // null terminator
	      *(LPTSTR) ((LPBYTE) lpCandList +
...
/**********************************************************************/
void PASCAL
AddCodeIntoCand (LPCANDIDATELIST lpCandList, WORD wCode)
{
  WORD wInCode;
  ...
  wInCode = HIBYTE (wCode) | (LOBYTE (wCode) << 8);
  {
    ...
    MultiByteToWideChar (NATIVE_ANSI_CP, 0, (LPCSTR) & wInCode, 2, &wUnicode,
			 1);
    *(LPUNAWORD) ((LPBYTE) lpCandList +
		  lpCandList->dwOffset[lpCandList->dwCount]) = wUnicode;
  }
  // null terminator
  *(LPTSTR) ((LPBYTE) lpCandList + lpCandList->dwOffset[lpCandList->dwCount] +
	     sizeof (WORD)) = TEXT ('\0');
...
/**********************************************************************/
void PASCAL
UnicodeAddCodeIntoCand (LPCANDIDATELIST lpCandList, WORD wCode)
{
...
/**********************************************************************/
void PASCAL
XGBAddCodeIntoCand (LPCANDIDATELIST lpCandList, WORD wCode)
{
  WORD wInCode;
...
Finalize (LPINPUTCONTEXT lpIMC,
	  LPCOMPOSITIONSTRING lpCompStr, LPPRIVCONTEXT lpImcP, WORD wCharCode)
{
...
/**********************************************************************/
void PASCAL
CompWord (			// compose the Chinese word(s) according to
	   // input key
	   WORD wCharCode,
