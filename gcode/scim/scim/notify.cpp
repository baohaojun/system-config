...
/**********************************************************************/
void PASCAL
CompCancel (HIMC hIMC, LPINPUTCONTEXT lpIMC)
{
  LPPRIVCONTEXT lpImcP;
	   ...
	   LPPRIVCONTEXT lpImcP, LPTSTR lpszRead, DWORD dwReadLen)
{
  ...
  CopyMemory ((LPTSTR) ((LPBYTE) lpCompStr + lpCompStr->dwCompReadStrOffset),
			 ...
			 DWORD dwCompLen, LPVOID lpRead, DWORD dwReadLen)
{
  ...
  fRet =
    SetString (hIMC, lpIMC, lpCompStr, lpImcP, (LPTSTR) lpRead, dwReadLen);
...
/**********************************************************************/
void PASCAL
NotifySelectCand (		// app tell IME that one candidate string is
		   // selected (by mouse or non keyboard action
		   // - for example sound)
		   HIMC hIMC,
		   LPINPUTCONTEXT lpIMC,
		   LPCANDIDATEINFO lpCandInfo, DWORD dwIndex, DWORD dwValue)
{
  LPCANDIDATELIST lpCandList;
  LPCOMPOSITIONSTRING lpCompStr;
  LPPRIVCONTEXT lpImcP;
		...
		if (lpImcP->fdwImeMsg & MSG_ALREADY_OPEN)
		  {
		    lpImcP->fdwImeMsg =
		      (lpImcP->
		       fdwImeMsg | MSG_CLOSE_CANDIDATE) & ~(MSG_OPEN_CANDIDATE
							    |
