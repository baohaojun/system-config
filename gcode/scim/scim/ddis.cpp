...
/**********************************************************************/
BOOL FAR PASCAL
ImeSetDlgProc (			// dialog procedure of configuration
		HWND hDlg, UINT uMessage, WORD wParam, LONG lParam)
{
  ...
  switch (uMessage)
	...
	DrawConvexRect (GetDC (hDlg),
			rc.left + 7,
			rc.top + 7, rc.right - 7 - 1, rc.bottom - 40 - 1);
...
{*/
BOOL WINAPI
ImeConfigure (			// configurate the IME setting
	       HKL hKL,		// hKL of this IME
	       HWND hAppWnd,	// the owner window
...
/**********************************************************************/
DWORD PASCAL
XGBConversion (LPCTSTR lpszReading, LPCANDIDATELIST lpCandList, UINT uBufLen)
{
  UINT MAX_COMP;
  UINT uMaxCand;
  UINT iRet;
...
***************************************************************************/
BOOL
DBCSToGBCode (WORD wCode, TCHAR AbSeq[5])
{
  WORD AreaCode;

  //Converte Unicode to GBK
  // change CP_ACP to 936, so that it can work under Multilingul Env.
  WideCharToMultiByte (NATIVE_ANSI_CP, WC_COMPOSITECHECK, (LPCWSTR) & wCode,
		       1, (char *) &AreaCode, 2, NULL, NULL);
  wCode = AreaCode;
...
***************************************************************************/
BOOL
AreaToGB (TCHAR AbSeq[5], TCHAR GbSeq[5])
{
			  ...
			  LPCANDIDATELIST lpCandList, UINT uBufLen)
{
  ...
  lpCandList->dwSize = sizeof (CANDIDATELIST) +
    sizeof (DWORD) * uMaxCand + (sizeof (TCHAR) * nMaxKey + sizeof (TCHAR));
...
/**********************************************************************/
BOOL WINAPI
ImeDestroy (			// this dll is unloaded
	     UINT uReserved)
{
  if (uReserved)
    {
...
LRESULT WINAPI
ImeEscape (			// escape function of IMEs
...
/**********************************************************************/
void PASCAL
InitCompStr (			// init setting for composing string
...
/**********************************************************************/
BOOL PASCAL
ClearCompStr (LPINPUTCONTEXT lpIMC)
{
  HIMCC hMem;
  LPCOMPOSITIONSTRING lpCompStr;
  DWORD dwSize;
...
/**********************************************************************/
BOOL PASCAL
ClearCand (LPINPUTCONTEXT lpIMC)
{
  HIMCC hMem;
...
/**********************************************************************/
BOOL PASCAL
ClearGuideLine (LPINPUTCONTEXT lpIMC)
{
  HIMCC hMem;
...
/**********************************************************************/
void PASCAL
InitContext (LPINPUTCONTEXT lpIMC)
{
...
/**********************************************************************/
BOOL PASCAL
Select (HIMC hIMC, LPINPUTCONTEXT lpIMC, BOOL fSelect)
{
  LPPRIVCONTEXT lpImcP;
...
/**********************************************************************/
BOOL WINAPI
ImeSetActiveContext (HIMC hIMC, BOOL fOn)
{
  if (!fOn)
    {
    }
...
/**********************************************************************/
LONG
OpenReg_User (HKEY hKey,	// handle of open key 
	      LPCTSTR lpszSubKey,	// address of name of subkey to open 
	      PHKEY phkResult)	// address of handle of open key 
{
...
VOID
InfoMessage (HANDLE hWnd, WORD wMsgID)
{
  TCHAR szStr[256];
...
VOID
FatalMessage (HANDLE hWnd, WORD wMsgID)
{
  TCHAR szStr[256];
