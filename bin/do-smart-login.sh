...
xargs bash -x -c ' curl -A "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/31.0.1650.63 Safari/537.36" --request POST -c ~/.cache/system-config/logs/cookie2.txt -L -v "$@" "https://login.alibaba-inc.com/ssoLogin.htm?APP_NAME=smart&BACK_URL=http://smart.corp.taobao.com/&CONTEXT_PATH=site"' true > 1.html
of 1.html
